...
class PagamentoPS extends Conex {
...
// ambiente de produção real
    ...
    public function Pagamento($cliente = array(), $pedido = array(), $produtos = array()) {
        ...
        $dados['currency'] = 'BRL';
        $dados['reference'] = $pedido['refer']; // referencia pedido 
        $dados['redirectURL'] = Rotas::pag_pedido_retorno();
        $dados['notificationURL'] = Rotas::pag_pedido_retorno();
...
//$dados['shippingAddressComplement'] = '';
        ...
        foreach ($produtos as $item) {
            ...
            $dados['itemId' . $i] = $item['prod_id'];
            $dados['itemDescription' . $i] = $item['prod_nome'];
            $dados['itemAmount' . $i] = $item['prod_valor_us'];
            $dados['itemQuantity' . $i] = $item['prod_qtd'];
...
//  var_dump($xml);
                ...
                $this->psCod = $xml->code;
            }// fim do IF caso teve erros ou não 
        }  // fim do IF autorizado ou não 
    }
    ...
    public function pagamentoITENS($cliente = array(), $pedido = array(), $produtos = array()) {
        ...
        $dados['token'] = $this->token;
        $dados['currency'] = 'BRL';
        $dados['reference'] = $pedido['ped_refer']; // referencia pedido 
        $dados['redirectURL'] = Rotas::pag_pedido_retorno();
        $dados['notificationURL'] = Rotas::pag_pedido_retorno();
        $dados['senderName'] = $cliente['cli_nome'] . ' ' . $cliente['cli_sobrenome'];
        $dados['senderAreaCode'] = $cliente['cli_ddd'];
...
// fim do ISSET POST notificationType
    ...
    private function pedidoUpdate($codigo, $pago, $forma_pag, $referencia) {
        ...
        $query = "UPDATE {$this->prefix}pedidos SET ped_pgto_cod = :cod, ped_pgto_tipo = 'PAGSEGURO',";
        $query .= "ped_pgto_status = :pago, ped_pgto_forma = :forma  WHERE ped_refer = :ref";
